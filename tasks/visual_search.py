...
# Minimum recording time to make blink metrics usable even if target is found quickly
MIN_RECORD_DURATION_FOR_VALID = 12.0  # seconds
...
class VisualSearchConfig(TaskConfig):
    ...
    # Minimum time to keep recording (and to accept trial as valid for blink metrics)
    min_record_duration: float = MIN_RECORD_DURATION_FOR_VALID  # seconds
    ...
    # Minimum blinks required (QC flag only; we do NOT auto-fail just for low blinks)
    min_blinks: int = 3
...
class VisualSearchTask(BaseTask):
    ...
    def run_task(self) -> TaskResult:
        ...
        result.valid_rate = (
            result.n_valid_trials / result.n_trials if result.n_trials > 0 else 0.0
        )
        ...
        for t in trial_summaries:
            ...
            if flags:
                all_qc_flags.extend([f.strip() for f in flags.split(",") if f.strip()])

        # Determine session-level QC status
        if all(s == QCStatus.PASS for s in all_qc_statuses) and all_qc_statuses:
        ...
        high_confidence_trials = [
            t for t in trial_summaries if t.get("blink_rate_confidence") == "HIGH"
        ]
        ...
        if valid_trials:
            ...
            result.biomarkers = {
                # Blink rate (mean across valid trials; confidence tracked separately)
                "session_blink_rate_per_min": np.nanmean(
                    [t.get("blink_rate_per_min", np.nan) for t in valid_trials]
                ),
                "session_blink_rate_high_conf": np.nanmean(
                    [t.get("blink_rate_per_min", np.nan) for t in high_confidence_trials]
                )
                if high_confidence_trials
                else np.nan,
                # Blink duration (proxy)
                "session_blink_duration_proxy_ms": np.nanmean(
                    [t.get("blink_duration_proxy_mean_ms", np.nan) for t in valid_trials]
                ),
                # Interblink interval
                "session_interblink_interval_s": np.nanmean(
                    [t.get("interblink_interval_mean_s", np.nan) for t in valid_trials]
                ),
                "session_interblink_cv": np.nanmean(
                    [t.get("interblink_interval_cv", np.nan) for t in valid_trials]
                ),
                # Burstiness
                "session_blink_burstiness": np.nanmean(
                    [t.get("blink_burstiness", np.nan) for t in valid_trials]
                ),
                # Engagement
                "session_gaze_presence_pct": np.nanmean(
                    [t.get("gaze_presence_pct", np.nan) for t in valid_trials]
                ),
                # QC summary
    ...
    def _run_trial(self, trial_idx: int) -> Optional[Dict[str, Any]]:
        ...
        # IMPORTANT FIX:
        # Do NOT end trial when target is found. Continue recording until timer ends.
        while time.time() < trial_end and self.ui.running:
            ...
            if events["space"] and not target_found:
                # Mark found time, but keep recording (covert blink capture)
                target_found = True
            ...
            if target_found:
                self.ui.draw_text(
                    "Registered ✅ (keep looking until time ends)",
        ...
        if target_found:
            self._show_feedback("Registered!", success=True)
        else:
    ...
    def _compute_biomarkers(
        ...
        blink_rate_confidence = (
            "HIGH" if duration_s >= MIN_TRIAL_DURATION_FOR_HIGH_CONFIDENCE else "LOW"
        )
        ...
        duration_proxy_std = (
            np.std(valid_durations_ms) if len(valid_durations_ms) > 1 else np.nan
        )
        ...
        if n_blinks > 1:
            for i in range(1, len(blink_starts)):
                interval = (
                    blink_starts[i] - blink_ends[i - 1]
                    if i - 1 < len(blink_ends)
                    else blink_starts[i] - blink_starts[i - 1]
                )
                if interval > 0:
        ...
        gaze_presence = (valid_samples / total_samples * 100) if total_samples > 0 else 0.0

        # QC flags and status
        qc_flags: List[str] = []

        # Short duration => WARN (but we also hard-fail validity below if too short)
        if duration_s < MIN_TRIAL_DURATION_FOR_HIGH_CONFIDENCE:
            qc_flags.append("short_trial_duration")

        # Low blinks => WARN only (NOT FAIL)
        ...
        # Many invalid blink durations => WARN
        if n_blinks > 0 and duration_valid_count < n_blinks * 0.5:
            qc_flags.append("many_invalid_blink_durations")

        # Determine QC status:
        # FAIL only if tracking/system is bad (not low blinks).
        fail_flags = {"low_gaze_presence", "low_valid_fraction", "high_clamp_rate", "low_fps"}

        if len(qc_flags) == 0:
            qc_status = QCStatus.PASS
        elif any(f in fail_flags for f in qc_flags):
            qc_status = QCStatus.FAIL
        else:
            qc_status = QCStatus.WARN

        # Validity policy:
        # - Must be long enough to be meaningful for blink metrics
        # - Must not be a tracking/system FAIL
        valid = 1
        if duration_s < self.config.min_record_duration:
            valid = 0
            if "trial_too_short_for_validity" not in qc_flags:
                qc_flags.append("trial_too_short_for_validity")
            qc_status = QCStatus.FAIL
        elif qc_status == QCStatus.FAIL:
            valid = 0

        if valid == 0:
            if duration_s < self.config.min_record_duration:
                reason = InvalidReason.INSUFFICIENT_SAMPLES
            elif gaze_presence < 50 or generic_validity.valid_fraction < 0.7:
    ...
    def _create_skipped_trial(self, trial_idx: int, trial_start: float) -> Dict[str, Any]:
